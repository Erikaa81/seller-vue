...
interface StoreResponse {
  ...
  async function editStore(name_store: any, store_id: any): Promise<{ success: boolean; message?: string }> {
    const auth = new Auth();
    const currentUser = auth.currentUser();
    const token = currentUser?.token;
  ...
  async function getStore(store_id: any, store_name: any): Promise<{
store?: any; message?: string 
}> {
    const auth = new Auth();
    const currentUser = auth.currentUser();
    const token = currentUser?.token;
